...
public:
    ...
    BaseHandler(HandlerNameType name) : name(name) {
        static_assert(
            std::is_enum<HandlerNameType>::value,
            "Type HandlerNameType have to be based on the enum class");
        static_assert(
            std::is_same<typename std::underlying_type<HandlerNameType>,
                         uint32_t>::value,
            "Enum have to be base on uint32_t");
    }

    virtual std::shared_ptr<Response> getResponce(
